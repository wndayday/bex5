...
public class DialMeterChartDataConfigComposite extends Composite implements IChartComposite{
	...
	public DialMeterChartDataConfigComposite(Composite parent, int style ,XuiElement cSeriesE ,String [] dataIds,HashMap<String ,String []> dataMap) {
		super(parent, style);		
	...
	public void initSelfDefaultValue() {
		...
		XuiElement dialScaleE = configE.getChildByTagName("dial-scale");
		if(dialScaleE != null){
		...
		List<?> dialRangeList = configE.getChildListByTagName("dial-rangle");
		for(int i=0; i< dialRangeList.size(); i++){
			...
			team.add(new DataEntry("dial-rangle",lower,upper,c,"","",""));
...
//            		ttv.setInput(dataTableModel);
	...
	public void saveSelfConfig() {
		...
		configE.removeChildren("dial-scale");
		XuiElement dialScaleE = configE.getXuiDataModel().addElement("dial-scale", configE,null,null, "<dial-scale visible=\"true\" lower-bound=\"0\" upper-bound=\"100\" start-angle=\"-120\" extent=\"-300\" major-tick-increment=\"10\" minor-tick-count=\"4\"/>",null); 		
		...
		configE.removeChildren("dial-rangle");

		ArrayList<Team> teamList = dataTableModel.getTeamList();
		for(int i=0; i<teamList.size(); i++){
			Team team = teamList.get(i);			
			XuiElement dialRangeE = configE.getXuiDataModel().addElement("dial-rangle", configE,null,null, "<dial-rangle lower=\"\" upper=\"\" color=\"\" />",null); 
			...
			for(int j=0; j<dataEntryList.size(); j++){
				...
				String type = entry.getType();
				if(type.equalsIgnoreCase("dial-rangle")){
	...
	public void initLayout() {
		...
		combo_dataset_value.setToolTipText("刻度盘要分析的字段信息");
		...
		dial_scale_lower.setToolTipText("刻度盘起始坐标值");
		...
		dial_scale_upper.setToolTipText("刻度盘终止坐标值");
		...
		dial_rangle_lower.setToolTipText("刻度区域起始位置");
		...
		dial_rangle_upper.setToolTipText("刻度区域终止位置");
		...
		dial_rangle_c.setToolTipText("刻度区域颜色");
