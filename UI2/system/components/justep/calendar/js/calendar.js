...
(function($) {
	...
	//hcr support destroy for leak memory
	Calendar.prototype.destroy = function() {
		var items = [];
		items.push(this._weeks);
		items.push(this._sliders);
		items.push(this._downboxs);
		...
		$('.cal-cell').unbind('dbclick');
		$('a[data-event-id]', this.context).unbind("click");
		if (this.options.modal)
			$(this.options.modal).unbind('show.bs.modal').unbind('shown.bs.modal').unbind('hidden.bs.modal');
		$('.cal-month-box .cal-row-fluid').unbind('mouseenter').unbind('mouseleave');
		$('a.event').unbind('mouseenter').unbind('mouseleave');
		$('.cal-month-day, .cal-year-box .span3').unbind('mouseenter').unbind('mouseleave').unbind('click');
		this.options = null;
	};
