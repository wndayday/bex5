...
public class FileSelectView extends JProcessorImpl {
	...
	private String getModelPath(){
		return (JustepConfig.getHome() + "/model").replaceAll("\\\\", "/");
	}
	...
	private void run(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException {
		...
		extendConfigCatalog = getModelPath() + extendConfigCatalog;
		//选择的文件类型	
		String[] types = {};
			...
			if (!fileName.startsWith(".")) {
				...
				if (file.isDirectory()) {
					...
					if (items.size() > 0) {
						...
						jsonObject.put("value", file.getAbsolutePath().replaceAll("\\\\", "/").substring(modelPath.length()));
	...
	private void putTreeItem(JSONObject map, File file, String iconPath) {
		String modelPath = getModelPath();
		map.put("value", file.getAbsolutePath().replaceAll("\\\\", "/").substring(modelPath.length()));
