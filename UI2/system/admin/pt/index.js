define(function(require) {
	...
	require("css!./assets/admin/layout/css/themes/blue2").load({
		id : "style_color"
	});
	...
	var Model = function() {
		...
		this.on('onInit', function(event) {
			...
			event.config.needLogin = !(this.config && this.config.sessionId) && ('true'!==sessionStorage.getItem("isQuick"));
			event.config.main.url = '$UI/system/admin/pt/main/main.w';
			//event.config.loginURL = '$UI/system/designer/SA/pt/login/login.w';
			this.initPortal();
		});
		...
		$(".x-user")['true'!==sessionStorage.getItem("isQuick")?'show':'hide']();
	};
	...
	Model.prototype.setColor = function(options) {
		...
		// if (color == 'light2') {
		// $('.page-logo img').attr('src', Layout.getLayoutImgPath() +
		// 'logo-invert.png');
		// } else {
		// $('.page-logo img').attr('src', Layout.getLayoutImgPath() +
		// 'logo.png');
	...
	Model.prototype.doActivePage = function(event) {
		...
		if (!event.isMainPage) {
			...
			if (!event.isMainPage) {
				...
				if (!event.isMainPage) {
					$(".page-content").css("background-color", "#fff");/* 其它页白色背景 */
					$(".page-content>.page-title").show();/* 当前位置显示 */
					$('#portal-opened').show();
					$(this.comp("closeFuncBtn").domNode).show();
					if (event.first) {
						var html = justep.String
								.format(
										'<li><a data-pid="{0}" onmouseover="$(this).addClass(\'on\');" onmouseout="$(this).removeClass(\'on\');"><i class="opened-close pull-right icon-close-round"></i>{1}</a></li>',
										event.pageID, event.title);
					...
					$(this.comp("closeFuncBtn").domNode).hide();
			} else
				$(this.comp("closeFuncBtn").domNode).hide();
		} else {
			$(".page-content").css("background-color", "#eaeef1");/* 首页灰色背景 */
			$(".page-content>.page-title").hide();/* 当前位置隐藏 */
			$(this.comp("closeFuncBtn").domNode).hide();
		}
	};
	...
	Model.prototype.doCreateFunctionTree = function(event) {
		var funcs = event.funcs, $content = $(".page-sidebar-menu"), html = [], me = this;

		// html.push('<li><a href="#" isMain="true"><i class="fa fa-th-large"
		// /><span class="nav-label">首页</span></a></li>');
	...
	Model.prototype.doBeforeLogin = function() {
		$(".page-header-inner .menu-toggler").hide();
	...
	Model.prototype.doAfterLogin = function(event) {
		...
		$(".page-header-inner .menu-toggler").show();
	...
	Model.prototype.initPortal = function(event) {
		...
		$("#portal-opened .dropdown-menu").click(function(event) {
			if ($(event.target).hasClass('opened-close')) {
				...
				me.showPage($(event.target).data('pid'));
			}
		});
