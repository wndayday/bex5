define(function(require) {
	...
	Model.prototype._changeGridSize = function() {
		var grid = this.getElementByXid('grid');
		var height = $(this.getRootNode()).height() - $('.file-select-navbar').height() - 36 -4;
		var width = $(this.getRootNode()).width() - 56-4;
		$(grid).jqxGrid({
			width : width,
			height : height
		});
	...
	Model.prototype.modelLoad = function(event) {
		...
		if (!this.checkType) {
			this.checkType = this.getContext().getRequestParameters().checkType;
		}
		if (!this.rootDir) {
			this.rootDir = this.getContext().getRequestParameters().rootDir;
		}
		this._hideGrid(true);
