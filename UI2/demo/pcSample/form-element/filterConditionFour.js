define(function(require){
	...
	//设置初始值
	Model.prototype.init = function(output, css,num,index) {
		var checked;
		var obj = this.comps(output);
		for (var i = 0; i < obj.length; i++) {
			if (i != index) {
				$("."+css+":eq(" + i + ")").addClass("none");
			}else{
	...
	Model.prototype.modelLoad = function(event){
		this.init("deadlineOutput", "deadline",1,2);
		this.init("amountOutput", "amount", 2,1);
	};
	//js代码
	Model.prototype.jsBtnClick = function(event){
		util.showSource({
			self : this,
			data : "demo/pcSample/form-element/filterConditionFour.js"
		},true);
	};
	// 源码
	Model.prototype.sourceClick = function(event) {
		util.showSource({
			self : this,
			type : true,
			data : "demo/pcSample/form-element/filterConditionFour.w&xid=contents"
		}, true);
	};
	return Model;
});