...
public class DocLinkAttachments {
	...
	private void queryItems() {
		...
		container.getMetaData().setStoreByConcept("SA_DocNode", true);
	...
	public int saveTable() {
		...
		Table link = DocDBHelper.queryLink(billIDs);
		...
		for (Attachment att : atts.values()) {
			...
			if ("delete".equals(accessType)) {
				for (Iterator<Row> it = link.iterator(); it.hasNext();) {
					Row r = it.next();
				...
				att.setState(ModifyState.NONE);
				container.deleteRows(att.getsID());
				container.setRecordState(true);
			}
		}

		link.save(DocDBHelper.DATA_MODEL);
		if (dl.size() > 0) {
			dl.save();
		}
