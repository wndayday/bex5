...
namespace com.justep.Sample.WeiXin.Comm
{
    public class CustomMessageHandler : MessageHandler<CustomMessageContext>
    {
        ...
        private readonly string appId = ConfigHelper.AppConfig["appId"];
        private readonly string appSecret = ConfigHelper.AppConfig["appSecret"];
        ...
        public override sealed WeixinContext<CustomMessageContext, IRequestMessageBase, IResponseMessageBase> WeixinContext
        {
...
<a href=""http://weixin.senparc.com/FilterTest/Redirect"">点击这里</a>进行客户端约束测试（地址：http://weixin.senparc.com/FilterTest/Redirect），如果在微信外打开将重定向一次URL。";
            ...
            else if (requestMessage.Content == "托管" || requestMessage.Content == "代理")
            {
                ...
                var responseXml = this.RequestXml(agentUrl, agentToken, RequestDocument.ToString());
                //获取返回的XML
            ...
            else if (requestMessage.Content == "测试" || requestMessage.Content == "退出")
            {
                ...
                */
                ...
                {
                    //进入APP测试
                    responseMessage.Content = "您已经进入【Justep微信实例】的测试程序，请发送任意信息进行测试。发送文字【退出】退出测试对话。10分钟内无任何交互将自动退出应用对话状态。";
                }
                else
                {
                    //退出APP测试
                    responseMessage.Content = "您已经退出【Justep微信实例】的测试程序。";
                }
            }
...
授权之后，您的微信所收到的消息将转发到第三方（Justep微信实例）的服务器上，并获得对应的回复。
